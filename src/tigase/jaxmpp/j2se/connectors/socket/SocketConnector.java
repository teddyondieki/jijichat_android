...
public class SocketConnector implements Connector {
	...
	private class Reader {
		...
		private final ByteBuffer buf = ByteBuffer
				.allocate(DEFAULT_SOCKET_BUFFER_SIZE);
	...
	private class Worker extends Thread {
		...
		private final XMPPDomBuilderHandler domHandler = new XMPPDomBuilderHandler(
				new StreamListener() {
		...
		private final SimpleParser parser = SingletonFactory
				.getParserInstance();
		...
		public void run() {
			...
			int r = -2;
			try {
				while (connector.reader != null && !isInterrupted()
						&& (r = connector.reader.read(buffer)) != -1
						&& connector.getState() != Connector.State.disconnected) {
					parser.parse(domHandler, buffer, 0, r);
				}
				// if (log.isLoggable(Level.FINEST))
				log.finest(hashCode() + "Disconnecting: state="
						+ connector.getState() + "; buffer=" + r + "   " + this);
				if (!isInterrupted())
	...
	public static boolean isTLSAvailable(SessionObject sessionObject)
		...
		if (sf == null)
			return false;
		Element m = sf.getChildrenNS("starttls",
	...
	public static boolean isZLibAvailable(SessionObject sessionObject)
		...
		if (sf == null)
			return false;
		Element m = sf.getChildrenNS("compression",
	...
	public SocketConnector(Observable parentObservable,
			SessionObject sessionObject2) {
	...
	public void addListener(EventType eventType,
			Listener<? extends ConnectorEvent> listener) {
	...
	@Override
	public XmppSessionLogic createSessionLogic(
			XmppModulesManager modulesManager, PacketWriter writer) {
		if (sessionObject
				.getProperty(InBandRegistrationModule.IN_BAND_REGISTRATION_MODE_KEY) == Boolean.TRUE) {
			log.info("Using XEP-0077 mode!!!!");
			return new SocketInBandRegistrationXmppSessionLogic(this,
					modulesManager, sessionObject, writer);
		} else
			return new SocketXmppSessionLogic(this, modulesManager,
					sessionObject, writer);
	}
	...
	protected void fireOnError(Element response, Throwable caught,
		...
		if (response != null) {
			List<Element> es = response
					.getChildrenNS("urn:ietf:params:xml:ns:xmpp-streams");
			if (es != null)
	...
	protected void fireOnStanzaReceived(Element response,
			SessionObject sessionObject) throws JaxmppException {
	...
	protected void onError(Element response, Throwable caught)
			throws JaxmppException {
		if (response != null) {
			Element seeOtherHost = response.getChildrenNS("see-other-host",
					"urn:ietf:params:xml:ns:xmpp-streams");
			if (seeOtherHost != null) {
				if (log.isLoggable(Level.FINE))
					log.fine("Received see-other-host="
							+ seeOtherHost.getValue());
	...
	protected void onResponse(final Element response) throws JaxmppException {
		synchronized (ioMutex) {
			if ("error".equals(response.getName())
					&& response.getXMLNS() != null
					&& response.getXMLNS().equals(
	...
	public void onTLSStanza(tigase.xml.Element elem) throws JaxmppException {
		// Log.d("proceedTLS()", elem.toString());
	...
	public void onZLibStanza(tigase.xml.Element elem) throws JaxmppException {
		if (elem.getName().equals("compressed")
				&& "http://jabber.org/protocol/compress"
						.equals(elem.getXMLNS())) {
			proceedZLib();
		} else if (elem.getName().equals("failure")) {
	...
	protected void proceedTLS() throws JaxmppException {
		...
		try {
			sessionObject.setProperty(Scope.stream, DISABLE_KEEPALIVE_KEY,
					Boolean.TRUE);
			TrustManager[] trustManagers = sessionObject
					.getProperty(TRUST_MANAGERS_KEY);
			...
			SSLSocket s1 = (SSLSocket) factory.createSocket(socket, socket
			...
			s1.addHandshakeCompletedListener(new HandshakeCompletedListener() {
				...
				public void handshakeCompleted(HandshakeCompletedEvent arg0) {
					...
					sessionObject.setProperty(Scope.stream, ENCRYPTED_KEY,
							Boolean.TRUE);
					ConnectorEvent event = new SocketConnectorEvent(
							EncryptionEstablished, sessionObject);
					try {
			...
			if (sessionObject.getProperty(SessionObject.USER_BARE_JID) != null) {
				hostname = ((BareJID) sessionObject
			...
			final HostnameVerifier hnv = sessionObject
			...
			if (hnv != null && !hnv.verify(hostname, s1.getSession())) {
				throw new javax.net.ssl.SSLHandshakeException(
						"Cerificate hostname '"
								+ hostname
								+ "' doesn't match domain name you want to connect '"
								+ s1.getSession().getPeerHost());
			}
			socket = s1;
		...
		try {
			...
			try {
				...
				if (f != null) {
					...
					writer = new DeflaterOutputStream(socket.getOutputStream(),
							compressor);
				}
				...
				try {
					...
					// compatibility
					Constructor<DeflaterOutputStream> flushable = DeflaterOutputStream.class
							.getConstructor(OutputStream.class, Deflater.class,
									boolean.class);

					// we need wrap DeflaterOutputStream to flush it every time
					// we are
					// writing to it
					writer = new OutputStreamFlushWrap(flushable.newInstance(
							socket.getOutputStream(), compressor, true));
			...
			final InflaterInputStream is = new InflaterInputStream(
					socket.getInputStream(), decompressor);
			reader = new Reader(is);

			sessionObject.setProperty(Scope.stream, Connector.COMPRESSED_KEY,
					true);
	...
	private void reconnect(final String newHost) {
		...
		try {
			...
			Object x1 = this.sessionObject
					.getProperty(Jaxmpp.SYNCHRONIZED_MODE);
	...
	public void removeListener(EventType eventType,
			Listener<ConnectorEvent> listener) {
	...
	public void restartStream() throws XMLException, JaxmppException {
		...
		final int clientVersion = sessionObject
				.getUserProperty(Constants.VERSION_CODE);
		sb.append("client-ver='" + clientVersion + "' ");

		final String clientKey = sessionObject
				.getUserProperty(Constants.CLIENT_KEY);
		sb.append("key='" + clientKey + "' ");

		// END ADDED

		final BareJID from = sessionObject
				.getProperty(SessionObject.USER_BARE_JID);
		String to;
		...
		synchronized (ioMutex) {
			if (writer != null)
				try {
					...
					try {
						SocketConnectorEvent event = new SocketConnectorEvent(
								StanzaSending, sessionObject);
	...
	protected void setStage(State state) throws JaxmppException {
		...
		this.sessionObject
				.setProperty(Scope.stream, CONNECTOR_STAGE_KEY, state);
		if (s != state) {
	...
	public void start() throws XMLException, JaxmppException {
		...
		if (sessionObject.getProperty(TRUST_MANAGERS_KEY) == null)
			sessionObject.setProperty(TRUST_MANAGERS_KEY,
					new TrustManager[] { dummyTrustManager });
		...
		if (sessionObject.getProperty(HOSTNAME_VERIFIER_DISABLED_KEY) == Boolean.TRUE) {
			...
			sessionObject.setProperty(HOSTNAME_VERIFIER_KEY,
		...
		try {
			...
			if (serverHost == null) {
				...
				List<Entry> xx;
				DnsResolver dnsResolver = UniversalFactory
						.createInstance(DnsResolver.class.getName());
				if (dnsResolver != null) {
			...
			if (log.isLoggable(Level.CONFIG))
				log.config("Whitespace ping period is setted to " + delay
						+ "ms");
	...
	public void startTLS() throws JaxmppException {
		if (writer != null)
			try {
				log.fine("Start TLS");
				DefaultElement e = new DefaultElement("starttls", null,
						"urn:ietf:params:xml:ns:xmpp-tls");
	...
	public void startZLib() throws JaxmppException {
		if (writer != null)
			try {
				log.fine("Start ZLIB");
				DefaultElement e = new DefaultElement("compress", null,
						"http://jabber.org/protocol/compress");
	...
	private void terminateStream() throws JaxmppException {
		...
		if (state == State.connected || state == State.connecting
			...
			log.fine("Stream terminate not sent, because of connection state=="
					+ state);
	}
