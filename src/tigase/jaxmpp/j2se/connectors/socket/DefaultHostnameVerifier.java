...
public class DefaultHostnameVerifier implements HostnameVerifier {
	...
	protected boolean match(final String hostname, final String altName) {
		if (hostname == null || TextUtils.isEmpty(hostname) || altName == null
				|| TextUtils.isEmpty(altName))
			return false;
		...
		if (normalizedAltName.startsWith("*.")
				&& hostname.regionMatches(0, normalizedAltName, 2,
		...
		if (!hostname.regionMatches(suffixStart, normalizedAltName,
				asteriskIdx + 1, suffixLength)) {
			return false;
		}
		return true;
	}
	...
	protected boolean verifyHostname(String hostname,
			X509Certificate x509Certificate) throws CertificateParsingException {
	...
	protected boolean verifyIp(String ipAddr, X509Certificate x509Certificate)
