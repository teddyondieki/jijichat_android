...
public abstract class Socks5ConnectionManager implements ConnectionManager {
	...
	private static final Logger log = Logger
			.getLogger(Socks5ConnectionManager.class.getCanonicalName());
	...
	@Override
	public void addListener(final EventType eventType,
	...
	public static enum State {
		...
		Active, ActiveServ, Auth, AuthResp, Closed, Command, Welcome, WelcomeResp, WelcomeServ
	}

	public void discoverProxy(final JaxmppCore jaxmpp,
			final ConnectionSession session, final InitializedCallback callback)
			throws JaxmppException {
		session.setData(JAXMPP_KEY, jaxmpp);
		final DiscoItemsModule discoItemsModule = jaxmpp
				.getModule(DiscoItemsModule.class);
		JID jid = jaxmpp.getSessionObject().getBindedJid();
		discoItemsModule.getItems(JID.jidInstance(jid.getDomain()), null,
				new DiscoItemsModule.DiscoItemsAsyncCallback() {
					@Override
					public void onError(Stanza responseStanza,
							XMPPException.ErrorCondition error)
							throws JaxmppException {
						proxyDiscoveryError(jaxmpp, session, callback,
								"not supported by this server");
					}

					@Override
					public void onInfoReceived(String attribute,
							final ArrayList<DiscoItemsModule.Item> items)
							throws XMLException {
						...
						if (all == 0) {
							...
							final DiscoInfoModule discoInfoModule = jaxmpp
									.getModule(DiscoInfoModule.class);
							final List<JID> proxyComponents = Collections
									.synchronizedList(new ArrayList<JID>());
							for (final DiscoItemsModule.Item item : items) {
								try {
									discoInfoModule.getInfo(
											item.getJid(),
											new DiscoInfoModule.DiscoInfoAsyncCallback(
													null) {
												protected void checkFinished() {
													int count = counter
															.addAndGet(1);
													if (count == items.size()) {
														proxyDiscoveryFinished(
																jaxmpp,
																session,
																callback,
																proxyComponents);
									...
									if (count == items.size()) {
										proxyDiscoveryFinished(jaxmpp, session,
												callback, proxyComponents);
									}
								}
							}
						}
					...
					public void onTimeout() throws JaxmppException {
						proxyDiscoveryError(jaxmpp, session, callback,
								"proxy discovery timed out");
					}
				});

	}

	protected void proxyDiscoveryFinished(JaxmppCore jaxmpp,
			ConnectionSession ft, InitializedCallback callback,
			List<JID> proxyComponents) {
		JID proxyJid = (proxyComponents == null || proxyComponents.isEmpty()) ? null
				: proxyComponents.get(0);
		ft.setData(PROXY_JID_KEY, proxyJid);
		callback.initialized(jaxmpp, ft);
	}

	protected void proxyDiscoveryError(JaxmppCore jaxmpp, ConnectionSession ft,
			InitializedCallback callback, String errorText) {
		log.log(Level.WARNING, "error during Socks5 proxy discovery = {0}",
				errorText);
		ft.setData(PROXY_JID_KEY, null);
		callback.initialized(jaxmpp, ft);
	}

	protected void connectToProxy(JaxmppCore jaxmpp, ConnectionSession session,
			String sid, ConnectionEndpoint host) throws IOException,
			JaxmppException {
		session.setData(JAXMPP_KEY, jaxmpp);
		InetSocketAddress address = new InetSocketAddress(host.getHost(),
				host.getPort());
	...
	protected List<Streamhost> getLocalStreamHosts(ConnectionSession session,
			String sid) throws JaxmppException {
		try {
			StreamhostsResolver streamhostsResolver = UniversalFactory
					.createInstance(StreamhostsResolver.class
							.getCanonicalName());
			...
			return streamhostsResolver.getLocalStreamHosts(session
					.getSessionObject().getBindedJid(), server.getPort());
		} catch (Exception ex) {
			throw new JaxmppException("problem in getting local streamhosts",
					ex);
		}
	}

	protected void handleConnection(ConnectionSession session, Socket socket,
		...
		while (state != State.Closed && state != State.Active
			...
			if (state == State.Welcome) {
				...
				if (log.isLoggable(Level.FINEST)) {
					log.log(Level.FINEST, "read data = {0} state = {1}",
							new Object[] { read, state.name() });
				}
				buf.flip();
			}
	...
	protected void requestActivate(final ConnectionSession session,
			final Socket socket) throws JaxmppException {
		...
		jaxmpp.getModule(Socks5BytestreamsModule.class).requestActivate(
				usedProxyJid, session.getSid(), session.getPeer(),
	...
	protected static State processData(ConnectionSession ft,
			SocketChannel socket, State state, ByteBuffer buf)
			throws IOException {
		if (buf != null && buf.hasRemaining()) {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"processing received data of size {0} bytes",
						buf.remaining());
			}
			...
			case WelcomeServ:
				...
				if (ver1 != 0x05) {
					...
					if (ok) {
						...
						if (log.isLoggable(Level.FINEST)) {
							log.log(Level.FINEST,
									"stopping service {0} after failure during WELCOME step",
									socket.toString());
						}
						socket.close();
						return State.Closed;
			...
			case Command:
				...
				if (cmd == 0x01 && atype == 0x03) {
					...
					if (ft == null) {
						...
						if (log.isLoggable(Level.FINEST)) {
							log.log(Level.FINEST,
									"stopping service {0} without file transfer",
									socket.toString());
						}
						socket.close();
						return State.Closed;
			...
			default:
				log.log(Level.WARNING,
						"wrong state, buffer has remainging = {0}",
						buf.remaining());
				buf.clear();
			}
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"after processing received data set in state = {0}",
						state);
			}
		}
		...
		if (state == State.Welcome) {
			...
			if (out.hasRemaining()) {
				log.log(Level.WARNING,
						"we wrote to stream = {0} but we have remaining = {1}",
	...
	protected static String generateHash(ConnectionSession session) {
		try {
			...
			String data = session.isIncoming() ? sid
					+ session.getPeer().toString()
					+ session.getSessionObject().getBindedJid().toString()
					: sid
							+ session.getSessionObject().getBindedJid()
									.toString() + session.getPeer();
			...
			if (log.isLoggable(Level.FINEST)) {
				log.finest("for "
						+ session.getSessionObject().getBindedJid().toString()
						+ " generated " + data + " hash = " + enc.toString());
			}
			return enc.toString();
	...
	protected void fireOnConnected(ConnectionSession session, Socket socket) {
		try {
			observable.fireEvent(CONNECTION_ESTABLISHED, new ConnectionEvent(
					CONNECTION_ESTABLISHED, session.getSessionObject(),
					session, socket));
		} catch (Exception ex) {
	...
	protected void fireOnFailure(ConnectionSession session) {
		try {
			...
			observable.fireEvent(CONNECTION_FAILED, new ConnectionEvent(
					CONNECTION_FAILED, session.getSessionObject(), session));
		} catch (Exception ex) {
	...
	protected static void registerSession(ConnectionSession session,
			String sid, Socks5ConnectionManager instance) {
		synchronized (sessions) {
			session.setData(SID_KEY, sid);
			String hash = generateHash(session);
			session.setData(Socks5ConnectionManager.class.getCanonicalName(),
					instance);
	...
	protected static void clearSessions() {
		synchronized (sessions) {
			for (ConnectionSession session : new HashSet<ConnectionSession>(
					sessions.values())) {
				Socks5ConnectionManager connectionManager = session
						.getData(Socks5ConnectionManager.class
								.getCanonicalName());
	...
	private class TcpServerThread extends Thread {
		...
		public TcpServerThread(int port /* , long timeout */)
				throws IOException {
		...
		public void shutdown() {
			synchronized (TcpServerThread.class) {
				...
				try {
					...
					log.log(Level.WARNING,
							"problem with closing server socket", ex);
				}
			}
		}
