...
public class PresenceModule extends AbstractStanzaModule<Presence> {
	...
	public PresenceModule(Observable parentObservable,
			SessionObject sessionObject, PacketWriter packetWriter) {
		...
		this.sessionObject.getPresence().setHandler(new Handler() {
			...
			@Override
			public void setPresence(Show show, String status, Integer priority)
	...
	private void fireContactChangedPresenceEvent(Presence presence)
			throws JaxmppException {
		PresenceEvent event = preparePresenceEvent(ContactChangedPresence,
				presence);
		observable.fireEvent(event);
	}
	...
	private PresenceEvent preparePresenceEvent(EventType type, Presence presence)
			throws XMLException {
	...
	public void process(final Presence presence) throws JaxmppException {
		...
		log.finest("Presence received from " + fromJid + " :: "
				+ presence.getAsString());
		if (fromJid == null)
			return;

		boolean availableOld = sessionObject.getPresence().isAvailable(
				fromJid.getBareJid());
		sessionObject.getPresence().update(presence);
		boolean availableNow = sessionObject.getPresence().isAvailable(
				fromJid.getBareJid());

		final StanzaType type = presence.getType();

		if (type == StanzaType.unsubscribed) {
			PresenceEvent event = preparePresenceEvent(ContactUnsubscribed,
					presence);
			observable.fireEvent(event);
		} else if (type == StanzaType.subscribe) {
			// subscribe
			log.finer("Subscribe from " + fromJid);
			PresenceEvent event = preparePresenceEvent(SubscribeRequest,
					presence);
			observable.fireEvent(event);
			...
			PresenceEvent event = preparePresenceEvent(ContactAvailable,
					presence);
			observable.fireEvent(event);
		} else if (availableOld && !availableNow) {
			// contact unavailable
			...
			fireContactChangedPresenceEvent(presence);
			PresenceEvent event = preparePresenceEvent(ContactUnavailable,
					presence);
			observable.fireEvent(event);
		} else {
	...
	public void sendInitialPresence() throws XMLException, JaxmppException {
		...
		if (sessionObject.getProperty(SessionObject.NICKNAME) != null) {
			presence.setNickname((String) sessionObject
	...
	public void setPresence(Show show, String status, Integer priority)
		...
		// Status, priority and nickname not needed at the moment
...
//		}
		...
		// process before sending presence. not needed for now to reduce packet size being exchanged
//		if (fireBeforePresenceSend(presence))
//			return;
	 ...
	 *            priority.
	 */
	public void setPresenceShow(Show show) throws XMLException, JaxmppException {
		Presence presence = Presence.create();
		presence.setShow(show);
...
//			return;
	 ...
	 *            priority.
	 */
	public void setPresenceStatus(String status) throws XMLException,
			JaxmppException {
		Presence presence = Presence.create();
		presence.setStatus(status);
