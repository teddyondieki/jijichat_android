...
public class MucModule extends AbstractStanzaModule<Stanza> {
	...
	private final static Criteria DIRECT_INVITATION_CRIT = ElementCriteria
			.name("message").add(
	...
	private final static Criteria MEDIATED_INVITATION_CRIT = ElementCriteria
			.name("message")
			.add(ElementCriteria.name("x",
					"http://jabber.org/protocol/muc#user"))
			.add(ElementCriteria.name("invite"));

	private final static Criteria MEDIATED_INVITATION_DECLINED_CRIT = ElementCriteria
			.name("message")
			.add(ElementCriteria.name("x",
					"http://jabber.org/protocol/muc#user"))
			.add(ElementCriteria.name("decline"));

	private final static Criteria ROOM_CONFIGURATION_CHANGED_CRIT = ElementCriteria
			.name("message")
			.add(ElementCriteria.name("x",
					"http://jabber.org/protocol/muc#user"))
			.add(ElementCriteria.name("status"));
	...
	public static final EventType RoomConfigurationChanged = new EventType();
	...
	public static final Integer STATUS__ROOM_CONFIGURATION_CHANGED = 104;
	...
	public MucModule(Observable parentObservable,
			final SessionObject sessionObject, PacketWriter packetWriter) {
		super(ObservableFactory.instance(parentObservable), sessionObject,
				packetWriter);
		parentObservable.addListener(Connector.StateChanged,
				new Listener<ConnectorEvent>() {

					@Override
					public void handleEvent(ConnectorEvent be)
							throws JaxmppException {
						Connector.State s = sessionObject
								.getProperty(Connector.CONNECTOR_STAGE_KEY);
						if (s == null || s == Connector.State.disconnected
								|| s == Connector.State.disconnecting) {
							onNetworkDisconnected();
						}
					}
				});

		AbstractRoomsManager cm = UniversalFactory
				.createInstance(AbstractRoomsManager.class.getName());
		...
		if (this.sessionObject instanceof AbstractSessionObject) {
			((AbstractSessionObject) this.sessionObject).addListener(
					AbstractSessionObject.Cleared,
					new Listener<ClearedEvent>() {

						@Override
						public void handleEvent(ClearedEvent be)
								throws JaxmppException {
							onSessionObjectCleared(be.getSessionObject(),
									be.getScopes());
						}
					});
		}
	...
	protected boolean checkElement(Element element) throws XMLException {
		...
		if (element.getName().equals("message") && type != null
	...
	public void declineInvitation(InvitationEvent invitation, String reasonMsg)
			throws JaxmppException {
		if (invitation.getInvitationType() == Type.mediated) {
			Message message = Message.create();
			message.setTo(JID.jidInstance(invitation.getRoomJID()));

			Element x = new DefaultElement("x", null,
					"http://jabber.org/protocol/muc#user");
			message.addChild(x);
	...
	private void fireJoinRequestedEvent(Presence element, String nickname,
			Room room) throws JaxmppException {
		MucEvent event = new MucEvent(JoinRequested, sessionObject);
		fireMucEvent(event, element, nickname, room, null, null);
	}

	private void fireMucEvent(MucEvent event, Presence element,
			String nickname, Room room, Occupant occupant, XMucUserElement xUser)
			throws JaxmppException {
		if (event == null)
			return;
	...
	private void fireNewRoomCreatedEvent(Presence element, String nickname,
			Room room, Occupant occupant) throws JaxmppException {
		MucEvent event = new MucEvent(NewRoomCreated, sessionObject);
		fireMucEvent(event, element, nickname, room, occupant, null);
	}

	private void fireYouJoinedEvent(Presence element, String nickname,
			Room room, Occupant occupant) throws JaxmppException {
	...
	public Map<BareJID, Room> getRoomsMap() {
		return this.roomsManager.getRoomsMap();
	}
	 ...
	 *            reason description
	 */
	public void invite(Room room, JID inviteeJID, String reason)
			throws JaxmppException {
		Message message = Message.create();
		message.setTo(JID.jidInstance(room.getRoomJid()));

		Element x = message.addChild(new DefaultElement("x", null,
				"http://jabber.org/protocol/muc#user"));
	...
	public void inviteDirectly(Room room, JID inviteeJID, String reason,
			String threadId) throws JaxmppException {
		Message message = Message.create();
	...
	public Room join(final InvitationEvent invitation, final String nickname)
			throws XMLException, JaxmppException {
		return join(invitation.getRoomJID().getLocalpart(), invitation
				.getRoomJID().getDomain(), nickname, invitation.getPassword());
	}

	public Room join(final String roomName, final String mucServer,
			final String nickname) throws XMLException, JaxmppException {
		log.finer("Joining room[1]: " + roomName + ", mucServer: " + mucServer
				+ ", nickname: " + nickname);
		return join(roomName, mucServer, nickname, null);
	}

	public Room join(final String roomName, final String mucServer,
			final String nickname, final String password) throws XMLException,
			JaxmppException {
		...
		Room room = this.roomsManager.createRoomInstance(roomJid, nickname,
				password);
		this.roomsManager.register(room);

		log.finer("Joining room[2]: " + room + ", roomsManager("
				+ roomsManager.getRooms().size() + "): "
	...
	public void leave(Room room) throws XMLException, JaxmppException {
		if (room.getState() == State.joined) {
			...
			presence.setTo(JID.jidInstance(room.getRoomJid(),
					room.getNickname()));
	...
	protected void onSessionObjectCleared(SessionObject sessionObject,
		...
		if (scopes != null && scopes.contains(Scope.session)) {
			for (Room r : roomsManager.getRooms()) {
				...
				for (Occupant occupant : ocs) {
					...
					r.remove(occupant);
					fireMucEvent(event, null, occupant.getNickname(), r,
							occupant, null);
				}
			}
		}
	...
	public void process(Stanza element) throws JaxmppException {
		...
		if (element instanceof Message
				&& ROOM_CONFIGURATION_CHANGED_CRIT.match(element)) {
			log.finer("Processing -- processRoomConfigurationChangedMessage");
			processRoomConfigurationChangedMessage((Message) element);
		} else if (element instanceof Message
				&& MEDIATED_INVITATION_DECLINED_CRIT.match(element)) {
			log.finer("Processing -- processInvitationDeclinedMessage");
			processInvitationDeclinedMessage((Message) element);
		} else if (element instanceof Message
				&& MEDIATED_INVITATION_CRIT.match(element)) {
			log.finer("Processing -- processMediatedInvitationMessage");
			processMediatedInvitationMessage((Message) element);
		} else if (element instanceof Message
				&& DIRECT_INVITATION_CRIT.match(element)) {
	...
	protected void processDirectInvitationMessage(Message message)
		...
		InvitationEvent event = new InvitationEvent(InvitationReceived,
				sessionObject);
	...
	protected void processInvitationDeclinedMessage(Message message)
		...
		final Element x = message.getChildrenNS("x",
				"http://jabber.org/protocol/muc#user");
	...
	protected void processMediatedInvitationMessage(Message message)
			throws JaxmppException {
		Element x = message.getChildrenNS("x",
				"http://jabber.org/protocol/muc#user");
	...
	protected void processRoomConfigurationChangedMessage(Message message)
			throws JaxmppException {
		...
		Element x = message.getChildrenNS("x",
				"http://jabber.org/protocol/muc#user");
		String status = getFirstChild(x, "status").getAttribute("code");

		MucEvent event = new MucEvent(RoomConfigurationChanged, sessionObject);
		...
		if (element.getType() == StanzaType.error) {
			...
			if (nickname != null) {
				...
				if (occupant != null) {
					...
					List<Element> chatStateElems = element
							.getChildrenNS(ChatState.XMLNS);
					if (chatStateElems != null && chatStateElems.size() > 0) {
						ChatState chatState = ChatState
								.fromElement(chatStateElems.get(0));
						occupant.setChatState(chatState);
						MucEvent chatStateChangedEvent = new MucEvent(
								OccupantChangedPresence, sessionObject);
						fireMucEvent(chatStateChangedEvent, null, nickname,
								room, occupant, null);
					}
				}
			}
	...
	protected void processPresence(Presence element) throws JaxmppException {
		...
		if (element.getType() == StanzaType.error
				&& room.getState() != State.joined && nickname == null) {
			room.setState(State.not_joined);
		...
		if (element.getType() == StanzaType.unavailable
				&& nickname.equals(room.getNickname())) {
			room.setState(State.not_joined);
		...
		MucEvent event;
		if ((presOld != null && presOld.getType() == null)
				&& presNew.getType() == StanzaType.unavailable && xUser != null
			...
			log.finer(element.getFrom() + " wants to change nickname to "
					+ newNickName);
		} else if (room.getState() != State.joined && xUser != null
				&& xUser.getStatuses().contains(110)) {
			room.setState(State.joined);
			event = new MucEvent(YouJoined, sessionObject);
			occupant.setPresence(element);
			room.add(occupant);
		} else if ((presOld == null || presOld.getType() == StanzaType.unavailable)
				&& presNew.getType() == null) {
			...
			occupant.setPresence(element);
			room.add(occupant);
		} else if ((presOld != null && presOld.getType() == null)
				&& presNew.getType() == StanzaType.unavailable) {
			occupant.setPresence(element);
			room.remove(occupant);
