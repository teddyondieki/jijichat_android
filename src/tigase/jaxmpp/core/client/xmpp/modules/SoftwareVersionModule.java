...
public class SoftwareVersionModule extends AbstractIQModule {
	 ...
	 */
	public static abstract class SoftwareVersionAsyncCallback implements
			AsyncCallback {
		 ...
		 */
		protected abstract void onVersionReceived(final String name,
				final String version, final String os) throws XMLException;
	}
	...
	private final Criteria CRIT = ElementCriteria.name("iq").add(
			ElementCriteria.name("query", "jabber:iq:version"));

	private final String[] FEATURES = new String[] { "jabber:iq:version" };

	public SoftwareVersionModule(SessionObject sessionObject,
	 ...
	 *            general callback
	 */
	public void checkSoftwareVersion(JID jid, AsyncCallback callback)
	...
	public void checkSoftwareVersion(JID jid,
			SoftwareVersionAsyncCallback callback) throws XMLException,
			JaxmppException {
	...
	protected void processGet(IQ element) throws XMPPException, XMLException,
		...
		query.addChild(new DefaultElement("name",
				name == null ? DEFAULT_NAME_VAL : name, null));
		query.addChild(new DefaultElement("version", version == null ? "0.0.0"
				: version, null));
		if (os != null)
