...
public class Chat {
	...
	private void sendChatState(ChatState state) throws XMLException,
			JaxmppException {
		...
		if (presenceStore == null
				|| !presenceStore.isAvailable(jid.getBareJid()))
			return;
	 ...
	 *            message to send.
	 */
	public Message sendMessage(String body, String messageID,
			StanzaType stanzaType) throws XMLException, JaxmppException {
		Message msg = Message.create();
		msg.setTo(jid);
		msg.setType(stanzaType);
		msg.setId(messageID);
		msg.setBody(body);
		...
		if (messageDeliveryReceiptsEnabled) {
			msg.addChild(new DefaultElement("request", null,
					MessageModule.RECEIPTS_XMLNS));
		}
	...
	/**
	 * Sends media message in current chat with supplied message-id. It uses
	 * correct interlocutor JID and thread-id.
	 * 
	 * @param body
	 *            message to send.
	 */
	public Message sendMediaMessage(String body, String messageID)
	...
	public void setLocalChatState(ChatState state) throws XMLException,
			JaxmppException {
		...
		if (!state.equals(this.localChatState)
				&& (localChatState != null || state != ChatState.gone)) {
	 ...
	 */
	public void setMessageDeliveryReceiptsEnabled(
			boolean messageDeliveryReceiptsEnabled) {
