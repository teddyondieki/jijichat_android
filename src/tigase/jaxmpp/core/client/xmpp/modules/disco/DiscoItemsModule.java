...
public class DiscoItemsModule extends AbstractIQModule {
	...
	public static abstract class DiscoItemsAsyncCallback implements
			AsyncCallback {
		...
		public void onSuccess(Stanza responseStanza) throws XMLException {
			final Element query = responseStanza.getChildrenNS("query",
			...
			for (Element i : ritems) {
				...
				to.setDescription(i.getAttribute("desc"));
				items.add(to);
			}
	...
	public DiscoItemsModule(SessionObject sessionObject,
	...
	public void getItems(JID jid, BareJID from, AsyncCallback callback)
			throws XMLException, JaxmppException {
		getItems(jid, JID.jidInstance(from), null, callback);
	}

	public void getItems(JID jid, BareJID from, DiscoItemsAsyncCallback callback)
			throws XMLException, JaxmppException {
		getItems(jid, from, (AsyncCallback) callback);
	}

	public void getItems(JID jid, JID from, String node, AsyncCallback callback)
			throws XMLException, JaxmppException {
		IQ iq = IQ.create();
		iq.setTo(jid);
	...
	protected void processGet(IQ element) throws XMPPException, XMLException,
		...
		for (Item it : event.items) {
			...
			// e.setAttribute("desc", it.getDescription());
