...
import co.jijichat.Features;

public class MobileModeFeature {

	public static final String MOBILE_OPTIMIZATIONS_ENABLED = Features.MOBILE_V1
			+ "#enabled";

	public static final String MOBILE_OPTIMIZATIONS_QUEUE_TIMEOUT = Features.MOBILE_V1
			+ "#presence_queue_timeout";

	private static final String TAG = "MobileModeFeature";

	public static void updateSettings(Account account, JaxmppCore jaxmpp,
			Context context) {
		AccountManager accountManager = AccountManager.get(context);
		String valueStr = accountManager.getUserData(account,
				MobileModeFeature.MOBILE_OPTIMIZATIONS_ENABLED);
		boolean mobileOptimizations = (valueStr == null || Boolean
	...
	private final JijichatService jaxmppService;

	private boolean mobileModeEnabled = false;

	private TimerTask setMobileModeTask;

	public MobileModeFeature(JijichatService service) {
		...
		if (enable) {
			setMobileModeTask = new TimerTask() {
				...
				public void run() {
					setMobileModeTask = null;
					try {
						setMobileMode(jaxmppService.getJaxmpp(), enable);
	...
	protected void setMobileMode(JaxmppCore jaxmpp, boolean enable)
			throws JaxmppException {
		if (jaxmpp.getSessionObject()
				.getProperty(Connector.CONNECTOR_STAGE_KEY) == Connector.State.connected) {
			...
			if (xmlns == null
					|| (enable && !((Boolean) jaxmpp.getSessionObject()
							.getProperty(MOBILE_OPTIMIZATIONS_ENABLED))))
				return;
			...
			if (Features.MOBILE_V1.equals(xmlns)) {
				Integer timeout = jaxmpp.getSessionObject().getProperty(
						MOBILE_OPTIMIZATIONS_QUEUE_TIMEOUT);
				if (timeout != null) {
