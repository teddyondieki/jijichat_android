...
import co.jijichat.security.SecureTrustManagerFactory;
import co.jijichat.security.SecureTrustManagerFactory.DataCertificateException;
...
public class TrustCertDialog extends DialogFragment {
	...
	private void reconnectAccount(final String account) {
		final JaxmppCore jaxmpp = ((MessengerApplication) getActivity().getApplication()).getJaxmpp();
		new Thread() {
			@Override
			public void run() {
				try {
					JijichatService.disable(jaxmpp.getSessionObject(), false);
					jaxmpp.login();
